...
class TestUriStringRepresentation:
    ...
    def test_equality_for_round_tripping(self) -> None:
        ...
        original = "path"
        assert Uri.parse(original) == Uri.parse(str(Uri.parse(original)))
...
class TestUriParsing:
    ...
    def test_parse_authority_variations(self) -> None:
        ...
        # Host with port
        uri = Uri.parse("//example.com:8080")
        assert uri.host == "example.com"
        assert uri.port == 8080

        # Userinfo with host
        uri = Uri.parse("//user@example.com")
        assert uri.userinfo == "user"
        assert uri.host == "example.com"

        # Full authority
        uri = Uri.parse("//user:pass@example.com:8080")
        assert uri.userinfo == "user:pass"
        assert uri.host == "example.com"
        assert uri.port == 8080
...
class TestUriQueries:
    ...
    def test_queries_method_returns_parameters(self) -> None:
        ...
        uri = Uri.parse("/api?active=true&limit=10&sort=")
        queries = uri.queries()
