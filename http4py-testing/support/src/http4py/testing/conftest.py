...
def hello_handler(request: Request) -> Response:
    return Response.of(OK).body_("Hello from http4py server!").header_("Content-Type", "text/plain")
...
def headers_handler(request: Request) -> Response:
    ...
    return Response.of(OK).body_(response_body).header_("Content-Type", "text/plain")


def post_users_handler(request: Request) -> Response:
    return Response.of(OK).body_("POST received").header_("Content-Type", "text/plain")
